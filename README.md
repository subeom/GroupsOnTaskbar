...
> **Security warning.** This step makes your machine trust a self-signed
> certificate that lives only on your computer. Any MSIX package signed with
> that same certificate will then install without further prompting, so treat
> the private key as sensitive and never share the generated `.pfx` or the
> certificate's private key. This flow is intended for local development only,
> not for distributing builds to other people. Remove the certificate when you
> are done:
>
> ```powershell
> Get-ChildItem Cert:\LocalMachine\TrustedPeople | Where-Object Subject -eq 'CN=Taskbar Groups Development' | Remove-Item
> Get-ChildItem Cert:\CurrentUser\My | Where-Object Subject -eq 'CN=Taskbar Groups Development' | Remove-Item
> ```
...
Project scaffolding, XAML templates, and the placeholder images under
`src/GroupsOnTaskbar.App/Assets/` were generated by the official Microsoft
`dotnet new winui` templates and remain subject to their own license terms.
Replace the placeholder artwork before distributing your own build.
